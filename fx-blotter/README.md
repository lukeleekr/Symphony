...
| `TryFxRequest` | UBS·GS FX 요청 | `Sell KRW 872,000,000 USD 20260611` (UBS)<br>`SELL KRW 1,125,136,816 USD 6/11/26` (GS)<br>`SELL KRW (Kospi) 6,062,500 USD 10/06/26` (GS) | `Buy/Sell` + 통화로 시작. 날짜는 `yyyymmdd`·슬래시·`SPOT` 표기 지원, 통화 뒤 주석 토큰 허용 |
| `TryUbsFxOrderList` | UBS FX 주문 리스트 | `11-Jun-26 BUY KRW 2,439,375,000.00 Sell USD No Round` | `dd-Mmm-yy` 날짜로 시작 |
| `TryGsProductOrder` | GS 주문 리스트 | `GSI USD KRW Buy 31,505,187.00 KRW 12-Jun-2026 069802080 ...` | 통화쌍 2토큰 + Buy/Sell + Qty + Qty통화 + 결제일 |
| `TryGsBackToBack` | GS 백투백 거래 | `19800000000 2026-06-05 3400000000 B KRW 4,000,000,000.00 1535.25 USD ...` | `B/S` 한 글자 + 통화 + 금액 + **환율** + 상대통화 → **블로터 제외** (아래 정책 참고) |
| `TryGsCreditDebit` | GS CREDIT/DEBIT 통지 | `2 CREDIT KRW 1,300,000,000 USD 5/21/2026 ...` | `CREDIT/DEBIT` + 통화 + 금액 + 날짜. **매수/매도 매핑 미확정 → 노란색 검수** (본문 방향 문장이 있으면 자동 적용) |
| `TryGsNoSide` | GS 방향 없는 표 | `KRW 859,000,000 USD 5/27/2026` | 통화 + 금액 + 상대통화 + 날짜 (방향 단어 없음). 본문 방향 문장 적용, 없으면 **노란색 검수** |
| `TryGeneric` | **미등록 포맷 폴백** | 날짜 + `BUY/SELL` + 통화 + 금액이 있는 모든 줄 | 휴리스틱 추정 → **노란색 표시** |

### 본문 방향 문장 자동 적용 (GS)

일부 GS 메일은 표에는 방향이 없고 **본문에 "Sell KRW"처럼** 적혀 있습니다.
이런 메일은 **본문의 방향 문장까지 같이 드래그해서 붙여넣으면** 매크로가 그 방향을 기억해
이후의 방향 없는 표 줄(`TryGsNoSide`)과 CREDIT/DEBIT 줄에 적용합니다 (태그 `GS-NOSIDE`, 노란색 없음).

- 방향 문장 인식 조건: 짧은 줄(12토큰 이하)에 `BUY` 또는 `SELL`이 정확히 1번 + 통화 토큰 + 숫자 없음
- 방향 문장 없이 표만 붙여넣으면: 금액을 **양수로 넣고 노란색 표시** (`NOSIDE?`, `CREDIT?` 태그) → 이메일 보고 부호 직접 확정
- ⚠ 서로 다른 메일을 한 번에 붙여넣을 때는 방향 문장이 그 아래 모든 방향 없는 줄에 적용되므로 순서에 주의

### 미확정: GS CREDIT/DEBIT의 매수/매도 매핑

`CREDIT KRW`가 고객 기준 매수(−)인지 매도(+)인지 아직 확정하지 못해서, 현재는 **양수 + 노란색 검수**로
들어갑니다. 매핑이 확정되면 `TryGsCreditDebit`의 `pendingSide = ""` 분기를 수정하면 됩니다.

### 슬래시 날짜 처리 (GS)

GS 메일에는 `6/10/2026`(MM/DD)과 `10/06/26`(DD/MM) 표기가 혼재합니다. 슬래시 날짜는
**M/D와 D/M 두 해석이 모두 가능하면 오늘과 가까운 쪽을 자동 선택**합니다 (동률이면 M/D 우선).
선택된 날짜가 TDY/TOM/SPOT 범위 밖이면 Value에 날짜가 그대로 표시되므로 그때 검수하면 됩니다.

### ⚠ 운영 정책: GS 백투백(Back-to-Back) 거래는 블로터에 넣지 않음

골드만 이메일 중 **환율(FX rate)이 이미 채워져 있는 거래**(예: `B KRW 4,005,376,488.00 1535.25 USD`)는
백투백 거래로, 부킹만 하면 되고 블로터에는 입력하지 않습니다. 매크로는 이런 줄을 인식해서
자동으로 건너뛰고, 변환 완료 메시지에 "백투백 거래 N건은 블로터에 넣지 않았습니다"로 알려줍니다.

> **나중에 이 정책이 바뀌면**: `FXBlotter.bas`의 `TryGsBackToBack` 함수와 `ConvertPaste`의
> `"B2B-SKIP"` 분기를 수정하면 됩니다. 해당 포맷에는 환율이 포함되어 있으므로 Customer Rate까지
> 자동 입력하도록 만들 수 있습니다.

- 헤더 줄, 서명, 결제지시문 등 거래가 아닌 줄은 자동으로 무시됩니다.
- Generic 폴백으로 파싱된 행은 **노란색으로 표시**되며, 금액·부호·날짜를 반드시 수동 확인해야 합니다.
- **구분 컬럼에 인식된 포맷명**(`UBS-STOCK`, `GENERIC` 등)이 미리 채워집니다 — 어떤 파서가 잡았는지 확인한 뒤 실제 구분 코드(1864, 821 등)로 덮어쓰면 됩니다.
...
- 새 포맷이 어떤 모습인지 모를 때는 일단 붙여넣어 보세요 — Generic 폴백이 잡으면 노란색 행으로 들어오고, 구분 칸에 `GENERIC`이 찍힙니다. 그 결과가 자주 틀리면 전용 파서를 만들 시점입니다.
...
잘못 변환했다면 `Paste` 시트의 **[실행취소] 버튼**으로 마지막 변환 1회분(추가된 행 전체)을 삭제할 수 있습니다.

- 매크로 실행 후에는 Excel 기본 `Ctrl+Z`가 동작하지 않아서 자체 구현된 기능입니다.
- **가장 최근 변환 1회분만** 되돌릴 수 있습니다 (여러 단계 불가).
- 변환 후 Blotter에서 직접 행을 추가/삭제했다면 삭제 범위가 어긋날 수 있으니, 확인 대화상자에 표시되는 행 범위와 미리보기를 보고 진행하세요.
